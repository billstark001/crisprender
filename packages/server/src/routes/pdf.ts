...
pdfRoutes.post('/generate', async (c) => {
  ...
  if (typeof injectAttribute !== 'undefined' && typeof injectAttribute !== 'boolean') {
    return c.json({ error: 'injectAttribute must be a boolean' }, 400);
  }
  if (typeof onRender !== 'undefined' && typeof onRender !== 'string') {
    return c.json({ error: 'onRender must be a string' }, 400);
  }
  ...
  try {
    const options = Object.fromEntries(
      Object.entries({ html, url, selector, scale, format, fitMode, viewportWidth, viewportHeight, waitAfterLoad, injectAttribute, onRender, pruneInvisible })
